...
def acceleration_jerk_direct(particles: Particles, softening: float =0.) \
    ...
    for i in range(N):
        for j in range(i+1,N):
            ...
            jerk_ij = np.zeros(3)
            jerk_ij[0] = -((vx/r**3)-(3*(np.dot(dx,vx)*dx)/r**5))
            jerk_ij[1] = -((vy/r**3)-(3*(np.dot(dy,vy)*dy)/r**5))
            jerk_ij[2] = -((vz/r**3)-(3*(np.dot(dz,vz)*dz)/r**5))
...
def acceleration_direct_vectorized(particles: Particles, softening: float =0.) \
    ...
    acc_x = np.sum(np.nan_to_num(dx*particles.mass/r**3),axis=1)
    acc_y = np.sum(np.nan_to_num(dy*particles.mass/r**3),axis=1)
    acc_z = np.sum(np.nan_to_num(dz*particles.mass/r**3),axis=1)
